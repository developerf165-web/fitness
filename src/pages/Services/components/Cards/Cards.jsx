...
export default function Cards({ item, onEdit, onDelete }) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden cursor-pointer text-white group aspect-[16/9] color-bg-mini-card transition-transform duration-300 hover:scale-[1.02] hover:shadow-lg">
      ...
      {!showFallback ? (
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center color-bg-card text-gray-300 z-10 animate-fadeIn">
          <ImageOff size={36} className="opacity-60 mb-2" />
        </div>
      )}
      ...
      {/* Матн ва нарх */}
      <div className="absolute bottom-0 left-0 right-0 z-20 flex items-stretch pb-1 p-5">
        <span className="block w-0.5 h-4 color-bg-accent"></span>
        <div className="pl-2">
          <h3 className="mb-1 font-medium">{item.title}</h3>
          <p className="text-sm mb-0 color-accent">
