...
export const updateTrainer = async (trainerId, formData) => {
  try {
    const apiData = new FormData();

    // Mapping: Frontend -> API Fields
    apiData.append('name', formData.name);
    apiData.append('surname', formData.lastName);
    apiData.append('phone', formData.phone);
    ...
    const response = await authApi.post(`/coach/update/${trainerId}`, apiData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
